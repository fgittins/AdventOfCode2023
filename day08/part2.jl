...
function count_steps_until_end_with_Z(node, instructions, network)
    i = 0
    while node[end] ≠ 'Z'
        i += 1
...
function count_steps_until_all_end_with_Z(instructions, network)
    counts = []
    for node ∈ keys(network)
        if node[end] ≡ 'A'
            push!(counts, count_steps_until_end_with_Z(node, instructions, network))
        end
    end
    lcm(counts...)
end
...
count_steps_until_all_end_with_Z(instructions, network)
